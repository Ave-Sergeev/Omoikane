...
2. Скачайте архив, соответствующий архитектуре вашей macOS (Apple Silicon или Intel).
3. Распакуйте архив и переместите бинарный файл куда вам удобно. Запустите его из терминала.

**⚠️ Примечание ⚠️**  
При первом запуске утилиты, macOS может вывести предупреждение о «непроверенном разработчике». Это стандартное поведение macOS для стороннего ПО — просто разрешите запуск в настройках (раздел "Конфиденциальность и безопасность").
...
Если параметр не указан, используются значения из config.yaml или значения по умолчанию.
...
Если стандартные настройки не дают нужного результата, необходимо «играться» с параметрами CLI-аргументов и настройками в файле config.yaml, подбирая рабочую комбинацию под конкретно ваш случай.
